...
export const {addTodo, addUser, changeAct, clearTodo, removeTodo, fetchTodo, subscribeUs} = todoSlice.actions
...
export const subscribeState = (name, email) => async dispatch => {
    try {
        await axios.post(process.env.NEXT_PUBLIC_URL + '/api/fetchSubscription',{
            name: name,
            email: email
        }).
        then(resp=>{
            if (resp.data.message == 'subscribed') {
                dispatch(subscribeUs(resp.data.duration, resp.data.email))
            }
        })
    } catch (error) {
        return console.log(error.message)
